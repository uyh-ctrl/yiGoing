...
export type UnifiedLoginResult = { accountType: 'ADMIN' | 'MEMBER'; token: string; tokenHead: string; displayName: string; avatar?: string }
export function unifiedLoginAPI(data: LoginParam) {
  return http<UnifiedLoginResult>({ method: 'POST', url: '/auth/login', data })
}
export function memberRegisterAPI(data: LoginParam & { phone: string }) {
  return http<void>({ method: 'POST', url: '/auth/register', data })
}
